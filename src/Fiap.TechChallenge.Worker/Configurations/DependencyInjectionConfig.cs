    ...
    internal static void RegisterMessageBroker(this IServiceCollection serviceProvider)
    {
        serviceProvider.AddMassTransit(busRegistrationConfigurator =>
        {
            busRegistrationConfigurator.UsingRabbitMq((context, cfg) =>
            {
                ...
                cfg.ReceiveEndpoint(massTransitQueueName, e => e.ConfigureConsumer<ContactInsertConsumer>(context));
                
            });
            
            busRegistrationConfigurator.AddConsumer<ContactInsertConsumer>();
        });
    }
}